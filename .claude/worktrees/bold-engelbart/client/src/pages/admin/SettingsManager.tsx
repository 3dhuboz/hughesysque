...
import { updateSettings as apiUpdateSettings, seedDatabase, migrateFromFirestore, uploadImage } from '../../services/api';
...
const compressImage = (base64Str: string, maxWidth = 700, quality = 0.5, forceJpeg = false) => {
    return new Promise<string>((resolve) => {
        const isPng = !forceJpeg && base64Str.startsWith('data:image/png');
        const img = new Image();
        img.src = base64Str;
        img.onload = () => {
...
const SettingsManager: React.FC<{ mode?: 'admin' | 'dev' }> = ({ mode = 'admin' }) => {
  ...
  // AI Image Prompt Modal State
  const [imagePromptModal, setImagePromptModal] = useState<{ field: keyof AppSettings; defaultPrompt: string } | null>(null);
  ...
  // Generic Image Generator — opens styled modal for description
  const handleGenerateImage = (field: keyof AppSettings, promptContext: string) => {
      setImagePromptText(promptContext);
      setImagePromptModal({ field, defaultPrompt: promptContext });
  };

  const handleConfirmGenerateImage = async () => {
      if (!imagePromptModal || !imagePromptText.trim()) return;
      const { field } = imagePromptModal;
      setImagePromptModal(null);
      setIsGeneratingImage(field as string);
      const prompt = `${imagePromptText.trim()} High quality, appetizing, professional photography, cinematic lighting.`;
      const base64 = await generateMarketingImage(prompt);
      if (base64) {
          // Compress and upload to R2 instead of storing base64 in D1
          const compressed = await compressImage(base64, 600, 0.4, true);
          try {
              const url = await uploadImage(compressed, String(field));
  ...
  const ImageSettingRow = ({ label, settingKey, prompt, maxWidth = 1200 }: { label: string, settingKey: keyof AppSettings, prompt: string, maxWidth?: number }) => {
      ...
      const handleUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
          ...
          reader.onloadend = async () => {
              ...
              const compressed = await compressImage(base64, maxWidth, 0.7, true);
              try {
                  const url = await uploadImage(compressed, String(settingKey));
                  setFormData(prev => ({ ...prev, [settingKey]: url }));
              } catch {
                  // Fallback to inline base64 if R2 upload fails
...
curl -X POST https://your-site.pages.dev/api/v1/seed`}
      ...
      {imagePromptModal && (
          <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setImagePromptModal(null)}>
              <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-lg shadow-2xl" onClick={e => e.stopPropagation()}>
                  <div className="p-6 border-b border-gray-800">
                      <div className="flex items-center gap-3 mb-1">
                          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-bbq-red to-orange-600 flex items-center justify-center">
                              <Wand2 size={20} className="text-white" />
                          </div>
                          <div>
                              <h3 className="text-white font-bold text-lg">Generate AI Image</h3>
                              <p className="text-gray-500 text-xs">Describe what you want — be specific for best results</p>
                          </div>
                      </div>
                  </div>
                  <div className="p-6">
                      <textarea
                          ...
                          rows={4}
                          className="w-full bg-black/60 border border-gray-700 rounded-xl p-4 text-white text-sm focus:border-bbq-gold focus:ring-1 focus:ring-bbq-gold/50 outline-none resize-none placeholder-gray-600"
                          placeholder="e.g. Smoked brisket on a wooden cutting board with sides..."
                          autoFocus
                          onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleConfirmGenerateImage(); } }}
                      />
                      <p className="text-gray-600 text-xs mt-2">Press Enter to generate, Shift+Enter for new line</p>
                  </div>
                  <div className="p-4 border-t border-gray-800 flex items-center justify-end gap-3">
                      <button
                          onClick={() => setImagePromptModal(null)}
                          className="px-5 py-2.5 rounded-xl text-gray-400 hover:text-white hover:bg-gray-800 transition text-sm font-medium"
                      >
                          Cancel
                      </button>
                      <button
                          ...
                          className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-bbq-red to-orange-600 text-white font-bold text-sm hover:opacity-90 transition disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2"
                      >
