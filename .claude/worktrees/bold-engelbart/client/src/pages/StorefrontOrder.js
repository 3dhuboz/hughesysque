...
import { ShoppingBag, Trash2, CheckCircle, Clock, User, Mail, Phone, AlertCircle, ArrowRight, Truck, Check, Plus, Minus, Flame, Snowflake, X, Package, MapPin, CreditCard, Lock, Info, Ticket, CalendarDays } from 'lucide-react';
...
const MenuItemCard = ({ item, onAdd }) => {
  ...
  return (
    <>
      <div className="bg-bbq-charcoal rounded-xl overflow-hidden border border-gray-800 hover:border-gray-600 transition shadow-lg group flex flex-row h-28">
        <div className="relative w-28 shrink-0 overflow-hidden">
          ...
          {item.isPack && (
            <div className="absolute bottom-1 left-1 bg-purple-900/90 text-white text-[9px] font-bold px-1.5 py-0.5 rounded border border-purple-500">PACK</div>
          )}
        </div>
        <div className="flex-1 p-3 flex flex-col justify-between min-w-0">
          <div>
            <div className="flex items-start justify-between gap-2">
              ...
              <span className="text-bbq-gold font-bold text-sm shrink-0">${item.price}</span>
            </div>
            <p className="text-xs text-gray-500 line-clamp-1 mt-0.5">{item.description}</p>
          </div>
          <div className="flex items-center justify-between gap-2 mt-1">
            <div className="flex items-center bg-black/40 rounded p-0.5 border border-gray-700">
              <button onClick={() => setQty(q => Math.max(1, q - 1))} className="w-6 h-6 flex items-center justify-center text-gray-400 hover:text-white rounded transition"><Minus size={12}/></button>
              <div className="w-6 text-center font-bold text-white text-xs">{qty}</div>
              <button onClick={() => setQty(q => q + 1)} className="w-6 h-6 flex items-center justify-center text-gray-400 hover:text-white rounded transition"><Plus size={12}/></button>
            </div>
            <button onClick={handleClick} disabled={isAdded}
              className={`py-1.5 px-4 rounded-lg font-bold text-xs transition flex items-center gap-1.5 ${isAdded ? 'bg-green-600 text-white' : 'bg-white hover:bg-gray-200 text-black'}`}>
              {isAdded ? <><Check size={12}/> Added</> : (item.isPack ? 'Customize' : 'Add')}
            </button>
          </div>
        </div>
      </div>
...
const StorefrontOrder = () => {
  ...
  useEffect(() => {
    if (selectedEvent) {
      ...
      let startMin = parseInt(selectedEvent.startTime?.split(':')[1] || '0');
      let endHour = parseInt(selectedEvent.endTime?.split(':')[0] || '18');
      let endMin = parseInt(selectedEvent.endTime?.split(':')[1] || '0');
            ...
            <section id="date-selection">
              ...
              {orderEvents.length === 0 ? (
                ...
                {selectedEvent && (() => {
                  ...
                  return (
                    <div className="mb-4 bg-bbq-charcoal border border-bbq-red/30 rounded-xl p-4 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg bg-bbq-red flex items-center justify-center shrink-0">
                          <CalendarDays size={20} className="text-white" />
                        </div>
                        <div>
                          <div className="text-[10px] text-gray-500 uppercase font-bold tracking-widest">Ordering For</div>
                          <div className="text-white font-bold">
                            {pickupDate.toLocaleDateString(undefined, { weekday: 'long', day: 'numeric', month: 'short' })}
                          </div>
                        </div>
                      </div>
                      <div className="text-right">
                        {selectedEvent.location && <div className="text-xs text-gray-400">{selectedEvent.location}</div>}
                        <button onClick={() => { setSelectedDayId(''); setPickupTime(''); }} className="text-[10px] text-bbq-red hover:text-red-400 font-bold uppercase mt-0.5">Change</button>
                      </div>
                    </div>
                  );
                })()}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {orderEvents.map(evt => {
                    ...
                    pickupDate.setDate(pickupDate.getDate() + 1);
                    const isSelected = selectedDayId === evt.id;
                    return (
                      <button key={evt.id} onClick={() => { setSelectedDayId(evt.id); setPickupTime(''); }}
                        className={`p-4 rounded-xl border transition-all flex items-center gap-4 group text-left ${isSelected ? 'border-bbq-red bg-bbq-red/10 shadow-[0_0_15px_rgba(217,56,30,0.2)]' : 'border-gray-800 bg-bbq-charcoal hover:border-gray-600'}`}>
                        <div className={`w-12 h-12 rounded-lg flex flex-col items-center justify-center shrink-0 ${isSelected ? 'bg-bbq-red text-white' : 'bg-gray-800 text-gray-400 group-hover:bg-gray-700'}`}>
                          <span className="text-[10px] font-bold uppercase leading-none">{pickupDate.toLocaleDateString(undefined, { month: 'short' })}</span>
                          <span className="text-lg font-bold leading-none">{pickupDate.getDate()}</span>
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="font-bold text-white text-sm group-hover:text-bbq-gold transition truncate">
                            {pickupDate.toLocaleDateString(undefined, { weekday: 'long' })}
                          </div>
                          {evt.location && <div className="text-xs text-gray-500 truncate">{evt.location}</div>}
                          <div className="text-[10px] text-gray-600 mt-0.5 flex items-center gap-1">
                            <Clock size={9}/> {evt.startTime || '11:00'} – {evt.endTime || '18:00'}
                          </div>
                        </div>
                        {isSelected && <CheckCircle size={20} className="text-bbq-red shrink-0"/>}
                      </button>
                    );
                  })}
                </div>
